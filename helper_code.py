...
# Get pitch status from patient data.
def get_pitch(data):
    pitch = next((line.split(': ')[1].strip() for line in data.split('\n') if line.startswith('#Systolic murmur pitch:')), None)
    if pitch is None:
        raise ValueError('No pitch available. Is your code trying to load labels from the hidden data?')
    return pitch
