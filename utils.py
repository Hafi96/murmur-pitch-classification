...
def convert_label_to_int(str_label):
    return pitch_mapping_str2int[str_label]

def convert_label_to_str(int_label):
    return pitch_mapping_int2str[int_label]

def get_pitch_locations(data):
    locations = None
    for l in data.split('\n'):
...
def load_recordings_with_labels(data_folder, included_labels=["Low","Medium","High"]):   
    patient_files_arr, recording_files, pitchs = [], [], []
    patient_files = find_patient_files(data_folder)
    
    for pf in patient_files:
        patient_data = load_patient_data(pf)
        patient_pitch = get_pitch(patient_data)

        # Skip patients whose murmur pitch is not in the included labels
        if patient_pitch not in included_labels:
            continue
        
        patient_pitch_idx = included_labels.index(patient_pitch)  # Assign index-based labels
        locations = get_locations(patient_data)
        pitch_locations = get_pitch_locations(patient_data)
        p_recordings = get_patient_recording_files(patient_data, len(locations))

        # Assign labels based on location rules
        for i in range(len(locations)):
            if included_labels[patient_pitch_idx] == "Low" and locations[i] not in pitch_locations:
                continue
            elif included_labels[patient_pitch_idx] == "Medium" and locations[i] not in pitch_locations:
                continue
            elif included_labels[patient_pitch_idx] == "High" and locations[i] not in pitch_locations:
                continue
            else:
                patient_files_arr.append(pf)
                recording_files.append(os.path.join(data_folder, p_recordings[i]))
                pitchs.append(patient_pitch_idx)
    ...
    pitchs = np.array(pitchs, dtype=np.int_)

    return patient_files_arr, recording_files, pitchs
